...
namespace caffe {
	...
	bool ReadProtoFromTextString(const string& str, Message* proto) {
		return google::protobuf::TextFormat::ParseFromString(str, proto);
	}

	class MemoryStreamBuf : public std::streambuf {
	public:
		MemoryStreamBuf(const void* pBuffer, int bufferSize);
	public:
	...
	MemoryStreamBuf::MemoryStreamBuf(const void* pBuffer, int bufferSize)
	...
	bool ReadProtoFromData(const void* data, int length, Message* proto) {
		MemoryStreamBuf buf(data, length);
		std::istream is(&buf);
		...
		if (!success)
		{
			IstreamInputStream* input = new IstreamInputStream(&is);
			input->Skip(4);
			GzipInputStream* coded_input = new GzipInputStream(input);
			success = proto->ParseFromZeroCopyStream(coded_input);
			delete input;
			delete coded_input;
		}
#endif
	...
	bool ReadProtoFromBinaryFile(const char* filename, Message* proto) {
		...
		{
			ZeroCopyInputStream* raw_input = new FileInputStream(fd);
			GzipInputStream* coded_input = new GzipInputStream(raw_input);
			success = proto->ParseFromZeroCopyStream(coded_input);
	...
	bool ReadRichImageToAnnotatedDatum(const string& filename,
		...
		// Read image to datum.
		bool status = ReadImageToDatum(filename, vector<float>(1, -1), height, width,
